...
from cn_clip.clip.model import Classifier, SemanticAttn
...
def main():
    ...
    vision_model_config_file = Path(__file__).parent.parent / f"/data/cn_clip/clip/model_configs/{args.vision_model.replace('/', '-')}.json"
    ...
    classifier_model = Classifier(512,args.num_class)
    attn_model = SemanticAttn()
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if args.distllation:
            num_steps_this_epoch = train(model, classifier_model,attn_model, data, epoch, optimizer, scaler, scheduler, args, steps, teacher_model)
        else:
            num_steps_this_epoch = train(model, classifier_model,attn_model, data, memory_dict, speaker_list, epoch, optimizer, scaler, scheduler, args, steps)
