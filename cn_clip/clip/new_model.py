...
# CLIP intent=False is_intent_loss=False
is_intent = False#True
is_summary = False
...
class Classifier(nn.Module):
    def __init__(self, input_size, num_classes):
        ...
        self.fc = nn.Linear(input_size*2, num_classes).cuda()
        self.fc1 = nn.Linear(input_size, num_classes).cuda()

    def forward(self, x,flag):
        if flag:
            intent_logits  = self.fc(x)
        else:
            intent_logits = self.fc1(x)
...
class CLIP(nn.Module):
    ...
    def forward(self, Flag, image,text,history, intent_features=None, mask_ratio=0):
        text = text.squeeze(1)
        history = history.squeeze(1)
        if Flag==1: # intent
            # print('history:',history)
            text_features = self.encode_text(text)
            history_features = self.encode_text(history)
            text_features = text_features / text_features.norm(dim=-1, keepdim=True)
            history_features = history_features / history_features.norm(dim=-1, keepdim=True)
            # print(text_features.size(),history_features.size())
            if is_summary:
                intent_features = torch.cat((text_features,history_features),dim=1)
                tmp = 1
            else:
                intent_features = text_features
                tmp = 0
            # exit()
            return intent_features, tmp
        elif Flag==0:
            # return self.encode_image(image)
            # print('evalution:', image)
            image_features = self.encode_image(image, mask_ratio)
            image_features = image_features / image_features.norm(dim=-1, keepdim=True)
            text_features = self.encode_text(text)
            before_dim = text_features.size()[1]
            after_dim = intent_features.size()[1]
            text_features = text_features / text_features.norm(dim=-1, keepdim=True)

            if is_intent:
                text_features = torch.cat((text_features,intent_features),dim=1)
                # print('text:',text_features.size(),intent_features.size())
                if is_summary:
                    linear_layer = nn.Linear(before_dim * 3, before_dim).cuda()
                    text_features = linear_layer(text_features.cuda())
                else:
                    linear_layer = nn.Linear(after_dim*2, before_dim).cuda()
                    text_features = linear_layer(text_features.cuda())
            # else:
                # linear_layer = nn.Linear(before_dim * 3, before_dim).cuda()
            # 应用线性变换

            # print(image_features.size(),text_features.size(),intent_features.size())
            # exit()
            ...
            return logits_per_image,logits_per_text,self.logit_scale.exp()

        else:
            img_dict = {}
            for id,img in image.items():
                # print('evalution:',id)
                image_features = self.encode_image(img, mask_ratio)
                image_features = image_features / image_features.norm(dim=-1, keepdim=True)
                img_dict[id] = image_features
            text_features = self.encode_text(text)
            before_dim = text_features.size()[1]
            after_dim = intent_features.size()[1]
            text_features = text_features / text_features.norm(dim=-1, keepdim=True)
            ...
            # print(text_features.size(), intent_features.size())

            # exit()
            return img_dict, text_features, self.logit_scale.exp()
