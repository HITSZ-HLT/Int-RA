...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        # self.sent_embeddings = nn.Embedding(config.type_vocab_size, config.hidden_size)
        # self.sentiment_embedding = nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=0)
        # self.LayerNorm is not snake-cased to stick with TensorFlow model variable name and be able to load
        # any TensorFlow checkpoint file
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)

    def forward(self, input_ids, token_type_ids=None, position_ids=None, sent_ids = None):
        ...
        # print('position id:',position_ids.size(),input_ids.size())
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask=None, head_mask=None):
        # print('input:',input_tensor.size(),attention_mask.size())
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, position_ids=None, sent_ids=None, head_mask=None):
        if attention_mask is None:
            attention_mask = torch.ones_like(input_ids)
        if token_type_ids is None:
            token_type_ids = torch.zeros_like(input_ids)
        if sent_ids is None:
            sent_ids = torch.zeros_like(input_ids)
        ...
        embedding_output = self.embeddings(input_ids, position_ids=position_ids, token_type_ids=token_type_ids, sent_ids = sent_ids)
